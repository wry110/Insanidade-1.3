...
export default function SaveLoadSystem({ character, setCharacter }: SaveLoadSystemProps) {
  ...
  const saveCharacter = () => {
    ...
    try {
      ...
      const newSavedCharacters = [
        ...
        { name: saveName, data: characterData },
      ]
  ...
  const loadCharacter = (savedData: string) => {
    try {
      ...
      if (parsedData.version !== migratedData.version) {
        alert(
          `Personagem migrado da versão ${parsedData.version || 1} para a versão ${migratedData.version}. Recomendamos salvar novamente.`,
        )
      } else {
  ...
  const exportCharacter = () => {
    try {
      ...
      const dataStr = JSON.stringify(characterToExport)
      ...
      alert("Erro ao exportar personagem.")
      console.error(error)
    }
  }
